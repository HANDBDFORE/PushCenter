...
public class AppRegisterSpringImpl implements AppRegister {
    ...
    public AppRegisterSpringImpl(List<AppChannel> appChannels) {
        check(appChannels);
        getLogger().debug(this.getClass() + " init, appChannels size: " + appChannels.size() + ", appChannels are: " + appChannels);
    ...
    public AppChannel loadApp(PushApp packetRequestApp) {
        getLogger().debug("I'm going to find appChannel by criteria: " + packetRequestApp);
        ...
        for (AppChannel appChannel : appChannels) {
            if (appChannel.getAppName().equals(packetRequestApp.getKey()) && appChannel.getAppSecret().equals(packetRequestApp.getSecret())) {
                getLogger().debug("found corresponding appChannel: " + appChannel);
                return appChannel;
            }

        }

        getLogger().error("Cannot find corresponding appChannel: " + packetRequestApp);
